    ...
    <div class="bg-gray-800 p-6 rounded-lg shadow-md border border-blue-500/20">
        <form action="{{ route('replies.update', $reply) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <textarea name="content" id="content" class="w-full py-2 px-3 rounded bg-gray-700 text-white focus:outline-none focus:ring-2 focus:ring-blue-500 border border-blue-500/30" rows="4" required>{{ $reply->content }}</textarea>
                @error('content')
            ...
            <div class="flex space-x-4">
                ...
                <a href="{{ route('replies.index') }}" class="bg-gray-500 hover:bg-gray-700 text-white px-4 py-2 rounded pixel-btn">Hủy</a>
            </div>
        </form>
    </div>
@endsection
