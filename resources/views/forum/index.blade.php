...
@section('title', 'Trang Chủ | 2D Game Hub - Diễn đàn game 2D')
...
@section('forum-content')
            ...
            <div class="md:w-1/2 mb-6 md:mb-0">
                ...
                <div class="flex space-x-4">
                    ...
                    <a href="{{ route('register') }}" 
                       class="btn-pixel bg-white text-blue-800 hover:bg-gray-200 px-6 py-2 font-medium"> {{-- Thêm btn-pixel và chỉnh màu nền/chữ --}}
                        Tham gia ngay
                    </a>
                    {{-- Nút "Tìm hiểu thêm" --}}
                    <a href="{{ route('forum.threads.index') }}" 
                       class="btn-pixel bg-transparent border-white text-white hover:bg-white hover:text-blue-800 px-6 py-2 font-medium"> {{-- Thêm btn-pixel và chỉnh màu nền/chữ/viền --}}
                        Tìm hiểu thêm
                    </a>
                </div>
            </div>
    ...
    <div class="bg-white rounded-lg shadow p-4 mb-6">
        <div class="border-b border-gray-200">
            <nav class="flex -mb-px">
                {{-- Các nút tab --}}
                <button role="tab" class="btn-pixel py-4 px-6 text-center text-sm active-tab 
                           border-transparent hover:bg-transparent"> {{-- Thêm btn-pixel, điều chỉnh border/bg --}}
                    <i class="fas fa-comments mr-2"></i>Thảo luận mới
                </button>
                <button role="tab" class="btn-pixel py-4 px-6 text-center text-sm text-gray-500 hover:text-gray-700 
                           border-transparent hover:bg-transparent"> {{-- Thêm btn-pixel, điều chỉnh border/bg --}}
    ...
    <div class="bg-white rounded-lg shadow p-4 mb-6 flex justify-between items-center">
        <div class="flex items-center w-full">
            @auth
                @if(Auth::user()->avatar)
                    <img src="{{ asset('storage/' . Auth::user()->avatar) }}" alt="{{ Auth::user()->user_name }}" class="rounded-full w-10 h-10 mr-3 object-cover">
                @else
        ...
        @auth
            {{-- Nút "Đăng" --}}
            <button onclick="window.location.href='{{ route('forum.threads.create') }}'" 
                    class="btn-pixel bg-blue-500 hover text-white px-4 py-2 rounded-full ml-2"> {{-- Thêm btn-pixel và chỉnh màu nền/chữ/bo góc --}}
    ...
    <div class="space-y-4">
            ...
            @foreach ($categories as $category)
                <div class="bg-white p-6 shadow-xl rounded-lg border border-blue-200 game-card">
                    <div class="flex items-center justify-between mb-3">
                        ...
                        {{-- Nút "Xem tất cả" cho danh mục --}}
                        <a href="{{ route('forum.categories.show', $category) }}" 
                           class="btn-pixel bg-gray-100 px-3 py-1 text-sm border-gray-300 hover:bg-gray-200 text-gray-700"> {{-- Thêm btn-pixel và chỉnh màu --}}
        ...
        <div class="text-center mt-6">
            ...
            <button id="load-more-posts" 
                    class="btn-pixel bg-white hover:bg-gray-100 text-blue-500 font-medium py-2 px-6 rounded-full border border-gray-300 shadow-sm"> {{-- Thêm btn-pixel và chỉnh màu --}}
